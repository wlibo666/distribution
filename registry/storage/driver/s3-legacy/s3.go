...
//
// This package leverages the goamz/goamz client library for interfacing with
// S3. It is intended to be used with old s3 compatible implementation such as ceph.
//
...
func FromParameters(parameters map[string]interface{}) (*Driver, error) {
	...
	if regionEndpoint == nil || fmt.Sprint(regionEndpoint) == "" {
		return nil, fmt.Errorf("No region endpoint parameter provided")
	}

	region := aws.Region{
		Name:       "s3",
		S3Endpoint: fmt.Sprint(regionEndpoint),
	}

	regionBucketEndpoint := parameters["regionbucketendpoint"]
	if regionBucketEndpoint == nil || fmt.Sprint(regionBucketEndpoint) == "" {
		region.S3BucketEndpoint = ""
	} else {
		region.S3BucketEndpoint = fmt.Sprint(regionBucketEndpoint)
	}
...
func (d *driver) Writer(ctx context.Context, path string, append bool) (storagedriver.FileWriter, error) {
	...
	if !append {
		...
		multi, err := d.Bucket.InitMulti(key, d.getContentType(), getPermissions())
		if err != nil {
			return nil, err
		}
...
func (w *writer) Write(p []byte) (int, error) {
	...
	if len(w.parts) > 0 && int(w.parts[len(w.parts)-1].Size) < minChunkSize {
		...
		multi, err := w.driver.Bucket.InitMulti(w.key, w.driver.getContentType(), getPermissions())
		if err != nil {
			return 0, err
		}
		w.multi = multi
